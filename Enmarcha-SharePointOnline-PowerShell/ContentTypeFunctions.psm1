	...
	Function New-SiteContentType()
{
    ...
    Process
    {
        ...
        {
            Write-Host -ForegroundColor Yellow  "El tipo de contenido '$ContentTypeId' ya existe"
            return
        }
		$parent = Get-PnPContentType | Where-Object { $_.Name -eq $ParentContentTypeName  }       
        if ($parent -eq $null)
        {
            Write-Host  -ForegroundColor Red "El tipo de contenido del que hereda '$InternalName' no existe"
            return
        }
		$parent = Get-PnPContentType | Where-Object { $_.Name -eq $Name  }   
		if ($parent -eq $null)
        {
		...
		{
				Write-Host -ForegroundColor Yellow  "Tipo de contenido '$ContentTypeId' Ya existe"		
		}
